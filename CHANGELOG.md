...
- **The PAM step died silently on Fedora — and its own safety net was unreachable.** The per-service PAM script ends with a cleanup loop whose last test legitimately fails on distros without `common-auth` (Fedora, Arch), making the script's exit status nonzero even on success; under `set -e` that killed `install.sh` mid-step-7 before the `rc` capture that was *designed* to downgrade PAM issues to a warning could ever run. The capture is now `set -e`-safe and the inner script exits 0 explicitly.
- **`plasmalogin` (the Plasma 6.7+ login manager that replaced sddm, shipped on Fedora 44 KDE) is now configured for fingerprint login.** It was missing from the service list entirely. The insertion follows the recipe @popy2k14 verified on Fedora 44 in #1 — `pam_unix` first so a typed password authenticates instantly, fingerprint as the fallback — inserted after `pam_selinux_permit`, which must stay first. (`plasmalogin` has no parallel password+fingerprint prompt yet — KDE tracks that upstream.)
- **Native-fingerprint detection now scans `/etc/pam.d` as well as `/usr/lib/pam.d`** — Fedora ships its `kde-fingerprint` service file in `/etc`, so the vendor-only check couldn't see it and the lock-screen location risked a duplicate line.
...
### Fixed — Arch and COSMIC, measured on a real Arch install for the first time
Both of this project's GitHub issues came from Arch/CachyOS users and it had never once run on Arch. An Arch VM with COSMIC 1.5.0 found four more problems.

- **The installer refused to run on Arch with the scanner plugged in.** Detection used `lsusb`, and Arch does not ship `usbutils` in a base install — the scanner was present and listed in sysfs, but with `lsusb` missing the check failed and the installer aborted saying no scanner was found. That check also runs before dependencies are installed, so it could never repair itself. Detection now reads `/sys/bus/usb/devices` directly and keeps `lsusb` only as a fallback; the GUI's scanner check got the same treatment.
- **Fingerprint was inserted above `pam_securetty` and `pam_nologin` on Arch — a security bug.** With no `common-auth` to anchor on, the old code fell back to inserting at line 1, and because the line is `sufficient`, a successful touch short-circuited the stack and skipped those checks: fingerprint login would sail past an `/etc/nologin` lockout. The insertion point is now computed — immediately before the include of the common stack where one exists, otherwise after the last `required`/`requisite` auth line, and only as a last resort at the top. The anchor also recognises Debian and Ubuntu's `@include` form, which the old literal match never caught.
- **COSMIC login was never configured, and the first attempt at fixing it was wrong.** COSMIC authenticates through greetd, which was missing from the login list entirely. But adding `/etc/pam.d/greetd` would have changed nothing: COSMIC ships `/etc/greetd/cosmic-greeter.toml` with `[general] service = "login"`, so user authentication goes through `/etc/pam.d/login`. The installer now reads the greeter's own config to learn the service name, and only when a greetd-family greeter is the active display manager — so a stray `/etc/greetd` cannot cause the console login stack to be edited on a machine that never uses it.
- **The installer now says so when it edits a keyring-bearing stack.** A `sufficient` fingerprint line short-circuits the module that captures the password to unlock the login keyring or wallet, which is how a fingerprint login can make every saved credential look erased. Where a dedicated fingerprint service exists we no longer touch those stacks at all; `plasmalogin` has none yet, so there the trade is now stated out loud with pointers to autokdewallet and the keyring helper.

### Measured on the Arch + COSMIC VM (for the record)
- **The v2.2.2 linker fix is confirmed on real Arch, not just inferred.** Stock Arch's `/etc/ld.so.conf` carries no `/usr/local` entry and `/etc/ld.so.conf.d/` is empty — exactly the silent-failure condition behind issue #2. The installer's fallback wrote its drop-in, after which the linker resolves the patched driver ahead of the distro copy, `ldd` shows the OpenCV libraries, and `fprintd` sees the scanner. `install.sh` completed all eight steps on Arch for the first time.
- **Enrolling a fingerprint through a GUI does not work on COSMIC 1.5.0.** polkit denies `net.reactivated.fprint.device.enroll` immediately and shows no dialog, on a session that is local and active with `cosmic-osd` running. Enrolment from a root shell works. This appears to be a COSMIC integration gap rather than a driver problem, and is being investigated upstream.
- **Fingerprint templates are portable between machines.** A template enrolled under `/var/lib/fprint/<user>/cs9711/` on one system was copied to another and recognised there — useful when reinstalling or moving to a new machine, and previously undocumented.
- **The scanner cannot complete an enrolment through QEMU USB passthrough.** It reports *Device disabled to prevent overheating* and drops off the bus, repeatably after four capture stages, and only a full guest reboot clears the firmware latch. Bare metal is unaffected. Anyone testing this driver in a VM should expect it.
